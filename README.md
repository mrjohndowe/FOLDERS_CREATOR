...
The installed **NSIS** version of the Reply Assistant checks the public GitHub Releases feed when it starts. If a newer release is available, it downloads it in the background and verifies it through Electron Builder’s update metadata. You can also choose **Help → Check for updates**. After a download, choose **Restart now** to install immediately, or **Install when I exit** to apply it the next time you close the Assistant normally. Portable `.exe` copies do not self-update; install the NSIS version to receive updates. The release label `v0.2.13` corresponds to the internal updater version and Windows build version `0.2.13`. This recovery release intentionally remains newer than the original `0.2.0` installer, so that installation can receive it through the updater. It accepts the inline-comment formatting accidentally written to `CHAT_AGE` by v0.2.6, allowing affected fresh installations to start without manually editing their settings. When automatic sending is armed, old review and held items are dismissed so they cannot block unread-user switching; newly generated replies that fail the readability check are skipped rather than held indefinitely. A delayed automatic reply reopens and verifies its intended conversation immediately before typing, so a Lovense jump back to the original chat does not silently cancel the reply. The monitor checks Lovense about every 250 milliseconds and immediately begins reply generation after capturing a new message; existing installations carrying the former 2500-millisecond default are automatically upgraded to the faster interval. Model requests use a bounded timeout for each individual attempt but keep retrying timeouts, network failures, rate limits, temporary server errors, and empty responses until a reply is returned or monitoring is paused. Permanent authentication and request errors still stop immediately. Manual **Place draft** and **Send now** actions reopen the review card's exact conversation before verifying, typing, or sending, so they continue to work after Lovense changes the selected user. If desktop startup fails for another reason, the installed app displays the error and saves `startup-error.log` in its private settings folder instead of exiting silently. Automatic sign-in fills the username and password separately, confirms both fields after Lovense has finished re-rendering, and only then submits the form.
...
The review dashboard includes dedicated **Settings** and **Saved conversations** pages. Settings is prefilled from the current private `config.ini` and saves selected identity, reply, and desktop-navigation settings back to that file without displaying API keys, access tokens, or a saved Lovense password. Reply Studio uses those saved identity, demographic, relationship, and system-prompt values directly instead of presenting duplicate persona, relationship, or tone fields. Saved conversations displays the assistant's local in-memory conversation history plus a private gallery of saved Lovense pictures. The text history resets when the server stops; the picture gallery persists in the local Assistant data folder until you remove those files.
